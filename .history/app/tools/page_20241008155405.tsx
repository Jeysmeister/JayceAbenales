...
const Page = () => {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="flex items-center justify-center w-full h-full -z-10">
      <Card className="p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-center items-center">
        {tools.map((tool, index) => {
          return <Tool tool={tool} index={index} key={index} />;
        })}
      </Card>
    </motion.div>
  );
